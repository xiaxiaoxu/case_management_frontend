<template>
  <div ref="caseModal">
    <a-modal
      :title="!case_id ? '新增用例' : '修改用例'"
      :width="'90%'"
      :getContainer="() => $refs.caseModal"
      style="top: 20px;"
      v-model="visible"
      @ok="handleOk"
      @cancel="handleCancel"
    ...
    >
      <div class="table-page-search-wrapper">
        <div style="width: 70%; height: 550px; overflow-x: hidden">
          <a-form :form="form">
            <a-row :gutter="4">
              <a-col :span="22">
              ...
              <a-popover v-model="templateListWindowVisible" title="模板" placement="bottomLeft" trigger="click" :bodyStyle="{padding: '0 10px 0 10px'}">
                <TemplateListWindow
                  slot="content"
                  ref="templateListWindowRef"
                  @handleChangeTemplate="handleChangeTemplate"
                  @templateListWindowInited="templateListWindowInitedFun"
                />
                <div class="select_template" @click="openTemplateListWindow">
                  <a-icon style="font-size: 14px; margin-right: 3px;" type="star" />
                  <span>选择模板</span>
                </div>
              </a-popover>
            </a-row>
            ...
            <a-row :gutter="4">
              <a-col :span="22">
                <a-form-item label="备注">
                  ...
                  ></Editor>
                  <!-- <a-textarea :rows="10" v-decorator="['remark', {rules:[{required: false, message: '请输入备注'}]}]" /> -->
                </a-form-item>
              </a-col>
            </a-row>
            <!-- <div style="position: absolute; bottom: -40px"> -->
            <a-row :gutter="4" style="position: absolute; bottom: -2px">
              <a-col :span="24">
                <a-form-item>
                  <a-checkbox v-if="!case_id" @change="onChangeCheckbox">
                    继续创建下一条
                  </a-checkbox>
                </a-form-item>
              </a-col>
            ...
            <div style="width: 80%; position: absolute; left: 75%; top: 10%">
                  ...
                  <a-form-item label="所属项目">
                    <a-select
                      v-decorator="[
                        'belong_project',
                        {
                          initialValue: this.project_name,
                  ...
                  <a-form-item label="维护人">
                    <a-input
                      v-decorator="[
                        'maintainer',
                        {
                          initialValue: userNameCn,
                          rules: [{ required: false, message: '请输入维护人' }]
                        }
                      ]"
                      placeholder="请输入维护人"
                    />
              ...
              <a-row style="margin-top: 10px">
                <a-col :md="6" :sm="24">
                  <span style="color: rgba(0, 0, 0, 0.85); font-size: 14px;">是否需要关联自动化：</span>
...
import { createOrModifyTestCase, getTestCaseById, getCaseTemplateById } from '@/api/case/case_two'
import { getModuleList, findModuleById } from '@/api/case/module_two'
import { TreeSelect } from 'ant-design-vue'
import Editor from './component/Editor'
import TemplateListWindow from './component/TemplateListWindow.vue'
...
export default {
  ...
  components: { TreeSelect, Editor, TemplateListWindow },
  data () {
    return {
      id: 0,
      keysList: [1],
      ...
      form: this.$form.createForm(this, {
        ...
        // mapPropsToFields: () => {
        //   return {
        //     case_title: this.$form.createFormField({
        //       value: this.case_title
        //     }),
        //     pre_condition: this.$form.createFormField({
        //       value: this.pre_condition
        //     }),
      ...
      templateListWindowVisible: false,
      module_list: [],
      module_value: '',
      project_members: [],
      project_desc: '',
  ...
  created () {
    ...
    console.log(document.documentElement.clientHeight || document.body.clientHeight)
    this.client_height = console.log(document.documentElement.clientHeight || document.body.clientHeight)
    this.getProjectNameList()
    // this.getModuleList()
  },
  computed: {
  ...mapGetters(['userNameCn'])
  },
  methods: {
    templateListWindowInitedFun () {
      console.log('in projectListWindowInited')
      console.log('this.projectListWindowVisible: ', this.templateListWindowVisible)
      this.templateListWindowInited = true
    },
    openTemplateListWindow () {
      console.log('templateListWindowInited in openTemplateListWindow: ', this.templateListWindowInited)
      if (this.templateListWindowInited === true) {
        this.$refs.templateListWindowRef.init()
      }
    },
    handleChangeTemplate (templateInfo) {
      console.log('templateInfo in handleChangeProject :', templateInfo)
      this.templateListWindowVisible = false
      // this.initialProjectName = projectInfo.project_name
      // this.project_id = templateInfo.id
      this.keysList = [1]
      this.stepData = []
      ...
      setTimeout(() => {
        ...
        this.form.setFieldsValue({
          case_title: templateInfo.case_name,
          case_type: templateInfo.case_type,
          case_level: templateInfo.case_level,
          maintainer: templateInfo.maintainer
        })
      }, 100)
      this.project_id = templateInfo.project_id
      this.module_id = templateInfo.module_id
      this.remark = templateInfo.remark
      this.case_type = templateInfo.case_type
      this.pre_condition = templateInfo.pre_condition
      this.remarkEditorContent = templateInfo.remark
      this.preConditionEditorContent = templateInfo.pre_condition
      // 重新渲染富文本组件
      ...
      this.getModuleList(templateInfo.project_id)
      getCaseTemplateById(templateInfo.id).then(res => {
        console.log('res.data in init: ', res.data)
        if (res.data) {
          this.stepInfo = res.data.step_info
          if (res.data.step_info.length >= 1) {
    ...
    init (caseId, projectId, projectName, moduleId) {
      console.log('in init function of CreateCaseForm')
      ...
      if (moduleId) {
        findModuleById(moduleId).then(res => {
      ...
      if (caseId) {
        this.loading = true
        getTestCaseById(caseId).then(res => {
          ...
          if (res.data) {
            ...
            setTimeout(() => {
              ...
              this.form.setFieldsValue({
                ...
                // pre_condition: res.data.pre_condition,
    ...
    handleOk () {
      this.form.validateFields((err, values) => {
        ...
        if (!err) {
          ...
          console.log(params)
          createOrModifyTestCase(params).then(res => {
