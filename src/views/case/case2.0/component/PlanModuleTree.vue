<template>
  <a-spin :spinning="loading" style="height: 200px;">
    ...
    <DeletePlanDetailModule
      ...
      @queryData="getPlanModuleListAndPlanCaseList"
    />
  </a-spin>
</template>
<script>
import { Tree } from 'ant-design-vue'
import { modifyModuleData, getPlanModuleList } from '@/api/case/module_two'
...
export default {
  ...
  methods: {
    ...
    getPlanModuleList () {
      ...
      console.log('params in getPlanModuleList:', params)
      getPlanModuleList(params).then(res => {
        this.moduleDataList = res.data
        console.log('this.moduleDataList in getPlanModuleList: ', this.moduleDataList)
        this.loading = false
      }).catch(err => {
        this.$message.error(err.message)
      })
    },
    getPlanModuleListAndPlanCaseList () {
      const params = {
        plan_id: this.planId
      }
      console.log('params in getPlanModuleList:', params)
