<template>
  <div ref="recycleCaseDetailModal">
    <a-modal
      :width="'90%'"
      :getContainer="() => $refs.recycleCaseDetailModal"
      v-model="visible"
      style="top: 20px;"
      @ok="handleOk"
      @cancel="handleCancel"
      ...
      <a-row :gutter="24">
        <a-col :md="34" :lg="17">
          <div style="height: 400px; overflow-x: hidden">
              ...
              <a-col :md="48" :lg="24">
                <a-card
                :body-style="{padding: '8px 10px 10px 8px'}"
                style="width:90%; margin-left: 5px;"
                >
                  <a-icon type="warning" :style="{ color: '#FAAD14' }" style="margin-left: 15px;"/>
                  <span style="margin-left: 10px;">{{ this.delete_person }}于</span>
                  <span style="margin-left: 5px;">{{ this.delete_time }}</span>
                  <span style="margin-left: 5px;">删除了测试用例</span>
                  <a style="margin-left: 50px;" @click="recoverCase">恢复</a>
                </a-card>
              </a-col>
            </a-row>
            ...
            <a-row :gutter="24" style="margin-top: 5px;">
              <a-col :md="24" :lg="12">
                <span style="font-size: 25px; bold-on">
                  {{ this.case_name }}
                </span>
              </a-col>
            </a-row>
            ...
            >
              ...
              <a-col :span="10">
                <a-textarea style="resize: none;" readOnly :autoSize="true" :value="stepData[keysList.indexOf(item)] ? stepData[keysList.indexOf(item)].step_desc : undefined"/>
              </a-col>
              <a-col :span="11">
                <a-textarea style="resize: none;" readOnly :autoSize="true" :value="stepData[keysList.indexOf(item)] ? stepData[keysList.indexOf(item)].step_result : undefined"/>
              </a-col>
            </a-row>
        ...
        <a-col :md="14" :lg="7">
          <div>
            <a-tabs default-active-key="1">
              <a-tab-pane key="1" style="height: 400px;">
                <span slot="tab">
                ...
                <div style="height: 20%;">
                  <CommentEditor
                    ...
                    style="margin-left: 10px; width: 92%; position: relative; z-index: 1"
                  />
                  <a-button
                    v-if="commentEditorVisible"
                    style="position: absolute; bottom: 50px; right: 40px; z-index: 2"
                    type="primary"
                    @click="addComment()"
                  >
                    发送
                  ...
                  <a-textarea
                    v-show="!commentEditorVisible"
                    style="margin-left: 10px; resize: none; height: 20px; width: 92%"
                    placeholder="发表评论"
                    @click="handleOpenCommentEditor()"
                  >
                  </a-textarea>
                </div>
              </a-tab-pane>
              <a-tab-pane key="2" style="height: 400px;">
                <span slot="tab">
                  操作记录
                </span>
...
import { getTestCaseById, getCaseCommentsByCaseId, addCommentToCase, deleteComment, getCaseOperationRecordByCaseId, recoverRecycleBinCase } from '@/api/case/case_two'
...
export default {
  props: {
    recycleCaseDetailVisible: {
      type: Boolean,
      default: false
    }
  },
  components: { CommentEditor, OperationRecordCompare },
  data () {
    return {
      id: 0,
      keysList: [1],
      ...
      stepTitle: '',
      visible: this.recyleCaseDetailVisible,
  ...
  watch: {
    ...
    recycleCaseDetailVisible (val) {
      console.log('##### in recycleCaseDetailVisible')
      this.visible = val
    }
  ...
  methods: {
    recoverCase () {
      ...
      this.$emit('changeRecycleCaseDetailVisible')
      const params = {
        case_id_list: [this.case_id]
      }
    ...
    init (caseInfo, projectName) {
      console.log('in init function of RecycleCaseDetail.vue ')
      console.log('caseInfo in init of RecycleCaseDetail.vue：', caseInfo)
      this.keysList = [1]
      this.stepData = []
      ...
      this.delete_person = caseInfo.delete_person
      this.delete_time = caseInfo.delete_time
